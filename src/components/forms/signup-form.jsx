...
const SignupForm = () => {
  ...
  const handleSubmit = async (values) => {
    setLoading(true);
    try {
      const res = await axiosInstance.post("/api/auth/register", values, { withCredentials: true });
      if (res.status === 201) {
        const { token, user: userData } = res.data;
        localStorage.setItem('accessToken', token);
        localStorage.setItem('user', JSON.stringify(userData));
      ...
      // Error handling is already in axios interceptor
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <Container className="d-flex justify-content-center align-items-center" style={{ minHeight: '90vh' }}>
      <Row className="p-4 shadow-lg rounded-4" style={{ backgroundColor: '#e0f7fa', maxWidth: '400px', width: '100%' }}>
        <Col>
          <h2 className="text-center mb-4 text-primary">Sign Up</h2>
          <Formik initialValues={initialValuesSignUp} validationSchema={validationSchemaSignUp} onSubmit={handleSubmit}>
            <Form>
              <TextField label='Name' type='text' placeholder='Enter name' name='name' />
              <TextField label='Email address' type='email' placeholder='Enter email' name='email' />
              <TextField label='Password' type='password' placeholder='Password' name='password' />
